...
class Nomina extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared("
          CREATE  OR REPLACE FUNCTION `crearfc` (IN `idarticulo` VARCHAR(5), IN `idfactura` INT, IN `cantidad` INT, IN `precio_compra` INT, IN `precio_venta` INT, IN `precio` INT)  NO SQL
          ...
          CREATE  OR REPLACE FUNCTION `crearfv` (IN `idarticulo` VARCHAR(5), IN `idfactura` INT, IN `cantidad` INT, IN `precio_venta` INT, IN `descuento` INT, IN `precio` INT)  NO SQL
          IF NOT EXISTS ( SELECT `idarticulo`, `idfactura`, `cantidad`, `precio_venta`, `descuento` 
          FROM detalle_factura
          WHERE detalle_factura.idarticulo = idarticulo and detalle_factura.idfactura=idfactura) THEN
          ...
          CREATE  OR REPLACE FUNCTION `cuentastotal` (IN `fecha` DATE)  NO SQL
